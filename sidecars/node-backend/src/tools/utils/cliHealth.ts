...
import type { CliHealthSnapshot, CliToolProbeSnapshot } from '../core/types.js';
...
function parseCliProbe(name: string, payload: unknown): CliToolProbeSnapshot {
  if (!isRecord(payload)) {
    return { name, available: false, error: 'invalid probe payload' };
  }
...
export async function getCliHealth(): Promise<CliHealthSnapshot> {
  const [opencodeResult, ohmyResult] = await Promise.allSettled([probeOpenCodeCli(), probeOhMyCli()]);
  ...
  const ohMyOpencode =
    ohmyResult.status === 'fulfilled'
      ? parseCliProbe('oh-my-opencode', ohmyResult.value)
      : {
          name: 'oh-my-opencode',
          available: false,
          error:
            ohmyResult.reason instanceof Error
  ...
  return {
    summary: { total: 2, available: availableCount, unavailable: 2 - availableCount },
    tools: { opencode, ohMyOpencode },
  };
}
